...
def serve(host: str, port: int, data_root: Path, *, ui_root: Path = PRODUCT_UI_ROOT) -> None:
    ...
    print(f"NEXUS offline-first product gateway listening on http://{host}:{port}", flush=True)
    server.serve_forever()


def main() -> None:
    parser = argparse.ArgumentParser(description="NEXUS offline-first canonical local product gateway")
