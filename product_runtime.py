...
PRODUCT_CONTRACT_VERSION = "nexus.product-runtime.v1"
...
def _paper_provenance(*, timeframe: str, at: str) -> dict[str, Any]:
    return {
        "kind": "manual",
        "source_id": "nexus-product-paper-terminal",
        "source_timestamp": at,
...
def _risk_policy() -> dict[str, Any]:
    return {
        "policy_id": "nexus-product-paper-risk",
        ...
        "max_stop_distance_fraction": "0.20",
...
def _position_exposure(state: PortfolioState, symbol: str | None = None) -> Decimal:
    ...
    for item_symbol, _side, quantity, entry in state.positions:
        if symbol is None or item_symbol == symbol:
            total += quantity * entry
    return total


def _risk_state(state: PortfolioState, *, symbol: str) -> dict[str, Any]:
    return {
        ...
        "signals_today": state.last_sequence,
        "seen_signal_ids": [],
...
def _risk_reducing_exit(
    *,
    ...
    """Deterministic paper-only exit gate.

    Risk-reducing close/reduce actions cannot expand authority or exposure. The
    execution layer still independently validates that the position exists,
    side matches, and quantity is a true reduction. Kill/session controls remain
    enforced by ``execute_paper_command``.
    """
    ...
    allowed = bool(
        ...
        and position[1] == side
        and requested_qty > 0
        and requested_qty <= position[2]
        and reference > 0
        and state.session_open
        and not state.kill_switch_enabled
    )
...
class ProductRuntime:
    """Local NEXUS product state with no live-trading authority.

    The only mutable financial surface is the deterministic demo/paper account.
    Events are append-only, hash chained, replayed on every load and persisted
    atomically. Corrupt state fails closed instead of silently resetting.
    """
    ...
    def _read_events(self) -> list[dict[str, Any]]:
        if not self.paper_events_path.exists():
            return []
        events: list[dict[str, Any]] = []
        try:
            with self.paper_events_path.open("r", encoding="utf-8") as handle:
                for index, line in enumerate(handle, start=1):
                    if index > MAX_PAPER_EVENTS:
                        raise ProductRuntimeError("paper event journal exceeds bounded limit")
                    if not line.strip():
                        raise ProductRuntimeError("paper event journal contains blank record")
                    value = json.loads(line)
                    if not isinstance(value, dict):
                        raise ProductRuntimeError("paper event journal record is not an object")
    ...
    def _write_events(self, events: list[dict[str, Any]]) -> None:
        if len(events) > MAX_PAPER_EVENTS:
            raise ProductRuntimeError("paper event journal exceeds bounded limit")
        replay(events)
    ...
    def _ensure_account(self) -> list[dict[str, Any]]:
        ...
        account = build_event(
            event_id="product:account:1",
            event_type="demo_account_opened",
            aggregate_id=PAPER_ACCOUNT_ID,
            sequence=1,
            occurred_at=at,
            correlation_id="product-bootstrap",
            causation_id="product-bootstrap-account",
            provenance=provenance,
            previous_event_digest=GENESIS_DIGEST,
            payload={"currency": PAPER_CURRENCY, "opening_cash": self.opening_cash},
        )
        session = build_event(
            event_id="product:account:2",
            event_type="session_boundary_recorded",
            aggregate_id=PAPER_ACCOUNT_ID,
            sequence=2,
            occurred_at=at,
            correlation_id="product-bootstrap",
            causation_id="product-bootstrap-session",
            provenance=provenance,
    ...
    def submit_paper_order(self, request: Mapping[str, Any]) -> dict[str, Any]:
        if not isinstance(request, Mapping) or set(request) != ORDER_KEYS:
            raise ProductRuntimeError("paper order schema mismatch")
        operation = str(request["operation"])
        symbol = str(request["symbol"]).upper()
        timeframe = str(request["timeframe"])
        side = str(request["side"])
        if operation not in {"open", "close", "reduce", "reverse"}:
            raise ProductRuntimeError("unsupported paper operation")
        if symbol not in PAPER_SUPPORTED_SYMBOLS:
            raise ProductRuntimeError("unsupported paper symbol")
        if timeframe not in PAPER_SUPPORTED_TIMEFRAMES:
            raise ProductRuntimeError("unsupported paper timeframe")
        if side not in {"long", "short"}:
            raise ProductRuntimeError("unsupported paper side")
        for name in ("quantity", "reference_price", "stop_price", "target_price"):
            value = request[name]
            if isinstance(value, float):
                raise ProductRuntimeError(f"{name} must not use binary floating point")
            try:
                numeric = Decimal(str(value))
            except Exception as exc:
                raise ProductRuntimeError(f"{name} is not a valid decimal") from exc
            if not numeric.is_finite() or numeric <= 0:
                raise ProductRuntimeError(f"{name} must be positive")

        with self._lock:
            events = self._ensure_account()
            state = replay(events).state
            at = _utc_now()
            signal_id = f"paper:{uuid.uuid4().hex}"
            correlation_id = f"product:{uuid.uuid4().hex}"
            ...
            if operation in {"close", "reduce"}:
                ...
                risk = evaluate_risk(signal, _risk_state(state, symbol=symbol), _risk_policy(), evaluated_at=at)
    ...
    def live_surface(self) -> dict[str, Any]:
        return {
            "contract_version": PRODUCT_CONTRACT_VERSION,
            "mode": "live",
            "status": "locked_owner_controlled",
            "enabled": False,
            "live_trading_authority": False,
