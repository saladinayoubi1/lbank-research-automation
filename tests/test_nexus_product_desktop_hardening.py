...
PRODUCT_JS = ROOT / "product_ui" / "product.js"
...
def test_status_bad_states_win_over_positive_substrings():
    text = PRODUCT_JS.read_text(encoding="utf-8")
    start = text.index("function statusClass(v)")
    end = text.index("function normalizeUIPreferences", start)
    block = text[start:end]
    assert block.index("locked|unavailable|denied|failed") < block.index("available|active|complete|ready")
    assert "quarantined" in block
    assert "pass|paper|verified" not in block


def test_dynamic_settings_navigation_uses_semantic_icon_not_numbering():
    text = PRODUCT_JS.read_text(encoding="utf-8")
    start = text.index("function ensureSettingsSurface()")
    end = text.index("function formUIPreferences", start)
    block = text[start:end]
    assert 'class="nav-icon" aria-hidden="true"' in block
    assert "<span>10</span>" not in block
    assert "n.textContent='11'" not in block
