...
from vol_adjusted_momentum_v1 import VolAdjustedMomentumError, _buy_hold_total_return, build_vol_adjusted_momentum_targets, run_vol_adjusted_momentum_robustness


def _frame(closes: list[float]) -> pd.DataFrame:
    close = pd.Series(closes, dtype="float64")
    return pd.DataFrame({"timestamp": pd.date_range("2026-01-01", periods=len(close), freq="4h", tz="UTC"), "open": close, "high": close + 1.0, "low": close - 1.0, "close": close, "volume": 1.0})
...
def test_buy_hold_benchmark_starts_at_first_research_open():
    benchmark_return = _buy_hold_total_return(_frame([100.0, 110.0, 121.0]), initial_cash=10_000.0, fee_bps=0.0, slippage_bps=0.0)
...
def test_robustness_contract_has_stress_benchmark_kill_gates_and_no_auto_promotion():
    ...
    assert set(result["kill_conditions"]) == {"all_stress_variants_inactive", "all_stress_variants_negative", "all_stress_variants_trail_buy_hold"}
