...
SUPPORTED_TIMEFRAMES_MS = {"1m": 60_000, "3m": 180_000, "5m": 300_000, "15m": 900_000, "30m": 1_800_000, "1h": 3_600_000, "2h": 7_200_000, "4h": 14_400_000, "6h": 21_600_000, "8h": 28_800_000, "12h": 43_200_000, "1d": 86_400_000, "3d": 259_200_000, "1w": 604_800_000}
REQUIRED_CANDLE_FIELDS = ("open_time_ms", "open", "high", "low", "close", "volume")
FORBIDDEN_METADATA_KEYS = {"access_token", "api_key", "apikey", "authorization", "cookie", "cookies", "credential", "credentials", "password", "private_key", "refresh_token", "secret", "token"}
...
def _normalize_candles(candles: Sequence[Mapping[str, Any]], *, interval_ms: int) -> list[dict[str, Any]]:
...
def _validate_window_completeness(*, retrieval_start_ms: int, retrieval_end_ms: int, interval_ms: int, normalized_candles: list[dict[str, Any]]) -> None:
...
def build_provenance_manifest(*, source: str, market_type: str, source_symbol: str, canonical_symbol: str, timeframe: str, endpoint_contract: str, mapping_policy_version: str, retrieval_start_ms: int, retrieval_end_ms: int, candles: Sequence[Mapping[str, Any]], metadata: Mapping[str, Any] | None = None) -> dict[str, Any]:
    ...
    _validate_window_completeness(retrieval_start_ms=retrieval_start_ms, retrieval_end_ms=retrieval_end_ms, interval_ms=interval_ms, normalized_candles=normalized_candles)
    ...
    manifest = {"schema": MANIFEST_SCHEMA, "source": source, "market_type": market_type, "source_symbol": source_symbol, "canonical_symbol": canonical_symbol, "timeframe": timeframe, "endpoint_contract": endpoint_contract, "mapping_policy_version": mapping_policy_version, "retrieval_window": {"start_ms": retrieval_start_ms, "end_ms": retrieval_end_ms}, "candle_count": len(normalized_candles), "first_open_time_ms": normalized_candles[0]["open_time_ms"], "last_open_time_ms": normalized_candles[-1]["open_time_ms"], "candles_sha256": _digest(normalized_candles), "metadata": metadata_value}
...
def validate_provenance_manifest(manifest: Mapping[str, Any], candles: Sequence[Mapping[str, Any]]) -> None:
    ...
    required = {"schema", "source", "market_type", "source_symbol", "canonical_symbol", "timeframe", "endpoint_contract", "mapping_policy_version", "retrieval_window", "candle_count", "first_open_time_ms", "last_open_time_ms", "candles_sha256", "metadata", "manifest_sha256"}
