...
def _require_public_repository(repository: str, token: str = "") -> None:
    value = _json_get(f"https://api.github.com/repos/{repository}", token)
    if value.get("full_name") != repository or value.get("private") is not False:
        raise RuntimeError("artifact transport is allowed only for the exact public repository")
...
def _artifact(
    ...
    _require_public_repository(repository, token)
    run = _json_get(f"https://api.github.com/repos/{repository}/actions/runs/{run_id}", token)
    if (
        ...
        raise RuntimeError("artifact run identity does not match exact main source")
    ...
    if len(matches) != 1:
        raise RuntimeError(f"expected exactly one current-run artifact, got {len(matches)}")
    artifact = matches[0]
...
def _download_outer(
    repository: str, artifact: dict[str, Any], output: Path, token: str
) -> None:
...
def _extract_exact_outer(outer: Path, root: Path, expected_names: set[str]) -> dict[str, Path]:
    ...
    with zipfile.ZipFile(outer) as archive:
        ...
        if len(names) != len(set(names)) or set(names) != expected_names:
            raise RuntimeError("artifact outer surface mismatch")
        for member in members:
            if member.is_dir() or Path(member.filename).name != member.filename:
                raise RuntimeError("artifact outer member is unsafe")
...
def restore_wheelhouse(
    *,
    ...
    if actual != expected_sha256:
        raise RuntimeError("wheelhouse deterministic archive SHA-256 mismatch")
    ...
    if (destination / "requirements.lock").read_bytes() != repository_lock.read_bytes():
        raise RuntimeError("wheelhouse requirements.lock mismatch")
...
def restore_historical(
    *,
    ...
    if actual != expected_sha256:
        raise RuntimeError("historical artifact SHA-256 mismatch")
    ...
    historical_artifact._validate_manifest(
        destination.resolve(),
    ...
    if verification.get("decision") != "pass":
        raise RuntimeError("historical snapshot verifier rejected artifact")
    return {
        "artifact_id": int(artifact["id"]),
        "archive_sha256": actual,
...
def main() -> int:
    ...
    if not token:
        raise RuntimeError(f"missing read-only GitHub token environment variable: {args.token_env}")
    ...
    if args.mode == "wheelhouse":
        ...
        result = restore_wheelhouse(
            repository_lock=Path(args.repository_lock), **common
        )
    else:
        digest = _sha(
            args.expected_snapshot_digest or "", _SHA64, "snapshot digest"
        )
