...
function Read-Manifest([string]$Name, [int]$ExactCount) {
    ...
    Require ($script:ManifestExpectedSha256.ContainsKey($Name)) 'UNKNOWN_BACKUP_MANIFEST'
    ExactHash $path $script:ManifestExpectedSha256[$Name] 'BACKUP_MANIFEST'
    $items = Get-Content -LiteralPath $path -Raw | ConvertFrom-Json
    Require ($items.Count -eq $ExactCount) 'BACKUP_MANIFEST_COUNT_MISMATCH'
    return $items
}
$script:ManifestExpectedSha256 = @{
    'product-data-sha256-manifest.json' = $ExpectedProductDataManifestSha256
    'shortcuts-manifest.json' = $ExpectedShortcutManifestSha256
    'global-paper-sync-manifest.json' = $ExpectedGlobalSyncManifestSha256
}
...
)][string]$ExpectedProductDataManifestSha256,
    [Parameter(Mandatory=$true)][ValidatePattern('^[a-fA-F0-9]{64}
    [Parameter(Mandatory=$true)][string]$StageProofPath,
    [string]$ProgramRoot = (Join-Path $env:LOCALAPPDATA 'Programs\NEXUS Personal Pro'),
    [string]$GlobalPaperSyncRoot = (Join-Path $env:LOCALAPPDATA 'NEXUS\paper-forward-sync'),
...
)][string]$ExpectedShortcutManifestSha256,
    [Parameter(Mandatory=$true)][ValidatePattern('^[a-fA-F0-9]{64}
    [Parameter(Mandatory=$true)][string]$StageProofPath,
    [string]$ProgramRoot = (Join-Path $env:LOCALAPPDATA 'Programs\NEXUS Personal Pro'),
    [string]$GlobalPaperSyncRoot = (Join-Path $env:LOCALAPPDATA 'NEXUS\paper-forward-sync'),
...
)][string]$ExpectedGlobalSyncManifestSha256,
