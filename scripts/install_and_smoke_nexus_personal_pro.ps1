...
function Remove-StaleVersionedInstalls([string]$ProgramRoot, [string]$CurrentInstallRoot, [int]$RetainPrevious = 2) {
    $root = (Get-FullPath $ProgramRoot).TrimEnd('\')
    $current = (Get-FullPath $CurrentInstallRoot).TrimEnd('\')
    if ($RetainPrevious -lt 0 -or $RetainPrevious -gt 5) { throw 'Rollback retention bound is invalid.' }
    if (-not (Test-PathWithin $current $root)) { throw 'Current install escaped the NEXUS program root.' }

    $versions = @(
        Get-ChildItem -LiteralPath $root -Directory -ErrorAction SilentlyContinue |
            Where-Object { $_.Name -match '^5\.1\.0-[0-9a-fA-F]{8}    if ($script:SmokeCleaned) { return }
...
try {
    ...
    if ($ActivateInstalledBuild -and $script:Evidence.final_launch.status -eq 'RUNNING_VISIBLE_ACTIVATED') {
        try {
            $cleanup = Remove-StaleVersionedInstalls -ProgramRoot $programRoot -CurrentInstallRoot $installRoot -RetainPrevious 2
            $script:Evidence.install.retained_rollback_versions = @($cleanup.retained)
            $script:Evidence.install.removed_stale_versions = @($cleanup.removed)
            $script:Evidence.install.cleanup_skipped_versions = @($cleanup.skipped)
            $script:Evidence.install.cleanup_status = if (@($cleanup.skipped).Count -gt 0) { 'PARTIAL' } else { 'PASS' }
            Write-Host ("NEXUS_VERSION_RETENTION={0} retained={1} removed={2} skipped={3}" -f
                $script:Evidence.install.cleanup_status,
                @($cleanup.retained).Count,
                @($cleanup.removed).Count,
                @($cleanup.skipped).Count)
        } catch {
            $script:Evidence.install.cleanup_status = 'SKIPPED_SAFE'
            $script:Evidence.install.cleanup_skipped_versions = @('retention_guard_error')
            Write-Warning 'NEXUS version cleanup was skipped safely; installation remains valid.'
        }
    }
    ...
    $older = @($versions | Where-Object { (Get-FullPath $_.FullName).TrimEnd('\') -ne $current })
    $retained = @($older | Select-Object -First $RetainPrevious)
    $candidates = @($older | Select-Object -Skip $RetainPrevious)
    $removed = @()
    $skipped = @()

    foreach ($candidate in $candidates) {
        $candidatePath = (Get-FullPath $candidate.FullName).TrimEnd('\')
        if (-not (Test-PathWithin $candidatePath $root)) {
            $skipped += "$($candidate.Name):outside_root"
            continue
        }
        try {
            Assert-NotReparsePoint $candidatePath 'Rollback candidate'
            $manifestPath = Join-Path $candidatePath 'install-manifest.json'
            if (-not (Test-Path -LiteralPath $manifestPath -PathType Leaf)) {
                $skipped += "$($candidate.Name):manifest_missing"
                continue
            }
            Assert-NotReparsePoint $manifestPath 'Rollback manifest'
            $manifest = Get-Content -LiteralPath $manifestPath -Raw | ConvertFrom-Json
            if ([string]$manifest.schema_version -ne 'nexus.windows-side-by-side-install.v2' -or
                [string]$manifest.version -ne '5.1.0' -or
                [string]$manifest.install_mode -ne 'persistent_unpacked' -or
                [string]$manifest.source_sha -notmatch '^[0-9a-fA-F]{40}    if ($script:SmokeCleaned) { return }
    foreach ($process in @(Get-NewNexusProcesses)) {
        try { if ($process.MainWindowHandle -ne 0) { [void]$process.CloseMainWindow() } } catch { }
    }
    $deadline = [DateTime]::UtcNow.AddSeconds(12)
...
try {
 ...
 -or
                ...
                $skipped += "$($candidate.Name):manifest_untrusted"
                continue
            }
            if (@(Get-InstalledNexusProductProcesses -ProgramRoot $candidatePath).Count -gt 0) {
                $skipped += "$($candidate.Name):active_process"
                continue
            }
            Remove-Item -LiteralPath $candidatePath -Recurse -Force -ErrorAction Stop
            $removed += $candidate.Name
        } catch {
            $skipped += "$($candidate.Name):cleanup_error"
        }
    }
