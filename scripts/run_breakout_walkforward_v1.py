...
from breakout_walkforward_v1 import run_breakout_walk_forward
from research_data import load_research_series

ROOT = Path(__file__).resolve().parents[1]
STATUS = ROOT / "data" / "market" / "_backfill_status.csv"
OUTPUT = ROOT / "build" / "research" / "breakout_walkforward_v1.json"
...
def select_research_series(status: pd.DataFrame, limit: int = 2) -> list[tuple[str, str]]:
    ...
    candidates = status.loc[
        ...
        & (pd.to_numeric(status["rows"], errors="coerce") >= 1300)
    ].copy()
...
def main() -> None:
    ...
    if not selected:
        raise RuntimeError(
            "no research-ready hour4 series with at least 1300 rows are available"
        )
    ...
    for symbol, timeframe in selected:
        ...
        result = run_breakout_walk_forward(frame)
        evidence.append(
            {
                "symbol": symbol,
                "timeframe": timeframe,
