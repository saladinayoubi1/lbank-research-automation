...
Correction and resolution: scheduled Paper runs `34384929765` and `34395196081` reached a verification-valid 8/12 `WAITING_FOR_FRESH_CELLS` snapshot, but the workflow incorrectly imposed the 12/12 `PAPER_LOOP_ACTIVE` threshold on that separate waiting state. Freshness is source-SHA-bound; immediately after a main SHA change, the four `hour4` cells correctly stay `SKIPPED_NO_NEW_BAR` until the next natural four-hour boundary while the eight `minute15`/`hour1` cells advance. HTTP 403 endpoint diagnostics were fallback observations, not the root cause of the deterministic four-cell shortfall. PR #1418 made the independent workflow verifier accept only strict engine-verified waiting snapshots below 12, with waiting mission truth and all downstream operational flags false, while preserving exact 12/12 for active status. Exact-main run `34397395277` then succeeded at 8/12 and persisted verified state artifact `10122430632` / `sha256:d18e52bc11c56cba6d4332c9b3681bf54b95478572bdeb8deafbe7a33f45cad6`. Persisting a complete verification-valid waiting snapshot is allowed; fabricating cells, relabeling waiting as active, silent exchange substitution, or weakening deterministic gates remains forbidden.
...
## 2026-09-09 — Strategy Factory replay transport is semantic, and v7 stops at human review
Status: ACTIVE

Evidence: PR #1421 registered Multi-Pair Discovery v2 as the eighth reviewed Strategy Factory stage. A subsequent rotation exposed that seven legacy search workflows and the Demo lifecycle bridge still depended on expired fixed artifact ID `8867026863`, which returned HTTP 410. PR #1422 repaired the lifecycle bridge, and PR #1424 applied the same manifest-bound replay-v2 selection contract across the seven legacy Strategy Factory workflows. Exact-main `50ac6ea8a331c08dca3e8be184ed0717380ff7f6` runs `34404429298`, `34404244591`, `34404244665`, `34404358221`, `34404651563`, `34404244680`, and `34404548937` succeeded. Neighborhood v7 artifact `10125093054` passed frozen-strategy and parameter-plateau validation with 29 dual-profile passers, while `automatic_paper_forward_started=false`.

Decision: Rotating GitHub Actions artifacts must be discovered from an unexpired prefix-matching set and accepted only after exact filename, delivery-manifest, zip, embedded-manifest, and canonical semantic-dataset identity validation. Never pin a rotating artifact ID as durable transport. An optional operator-supplied artifact ID is only a candidate filter and does not weaken validation. Treat the neighborhood-v7 result as eligible for independent human review only; it grants no automatic derivatives validation, prospective Paper-forward admission, candidate promotion, or Issue #984 mutation.

Boundary: Research/Backtest/Paper only. Deterministic Risk remains final. Live/L4, private credentials, real exchange orders, signing, deployment, production promotion, destructive authority, and self-authorization remain unavailable.

## 2026-09-09 — Isolated feedback jobs provision their own locked verifier runtime
Status: ACTIVE

Evidence: exact-main Multi-Pair Paper feedback run `34412163303` passed exact-SHA producer resolution and digest-bound artifact restore, then failed closed with `ModuleNotFoundError: No module named 'pandas'` while importing the official Paper snapshot verifier. The contract-test job had installed development dependencies, but GitHub Actions jobs are isolated. PR #1430 added pinned Python 3.12, `requirements.lock`, `pip check`, and ordering regression coverage to `link-exact-boundary-feedback`. All exact-head checks passed. On source `21af398b9e73896f98c792a99230cf1562f7a04b`, Paper run `34413401197` and seven-job Multi-Pair run `34413429714` succeeded; automatic feedback run `34414723066` then passed dependency provisioning, exact-SHA pair resolution and artifact binding before returning the expected `NO_OP_NOT_ELIGIBLE` at 4/12 fresh Paper cells.
...
## 2026-09-25 — Source-bound research fills and no-candidate exhaustion after execution repair
Status: ACTIVE

Evidence: PRs #1813, #1815 and #1816 merged only after exact-head checks, and the Lenovo regression suites passed. Old real MTF main run `36101958735` reproduced an unexecutable-dust zero-equity crash. Corrected exact-main run `36103201556` completed under source `6e1b27eb0f017e71844e5f0408b0355e93f392ef`; its verified evidence artifacts `10850451545` and `10850546396` certify 0 base/0 refined proposals in the exact corrected static search neighborhood. Separate corrected historical v2 main run `36102660134` produced artifact `10850490620`, failed both locked profiles and explicitly refused promotion.

Decision: Historical strategy fill counts, rankings, returns and candidate identities produced by the previous fee-funded rebalance executors are not interchangeable with new-source corrected backtests. Keep exact source/dataset/artifact provenance and the independent locked holdout boundary. The exhausted historical MTF search does not justify a repeated identical run or artificial extra trades. Register any new economic/low-turnover hypothesis before testing it, reserve genuinely uninspected future data for independent challenge, and admit any resulting strategy to a *new* prospective Paper window only after the existing human-review/Deterministic Risk boundaries. Do not touch the terminal failed #984 candidate or lower its frozen criteria.

Boundary: Research/Backtest/Paper only. Live/L4, real exchange orders, private credentials, production/deployment, withdrawal, signing and automatic strategy promotion remain disabled. Desktop GUI provenance remains its separately verified installed `77d23868` build, not the current repository documentation or research SHA.
