...
## Fallback
DeepSeek failure must not block NEXUS. Autonomous orchestration falls back to no AI-proposed task when the provider is unavailable or budget-blocked. Repository queue tasks remain authoritative and deterministic.
...
## Rollback
Rollback provider code, this ADR, pricing version/table, ledger schema, workflow integration, orchestrator integration and tests as one tuple. Paid autonomous planning is disabled in the rollback state. Preserve audit/usage records; never silently reset spend.
