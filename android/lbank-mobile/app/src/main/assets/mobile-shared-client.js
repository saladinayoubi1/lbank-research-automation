(()=>{
'use strict';
let snapshot=null,received=0,inflight=false,epoch=0,connection='offline';
...
async function refresh(){
  ...
  inflight=true;const requestEpoch=epoch;connection='loading';paint();
  try{
    ...
    snapshot=payload.shared_portfolio;received=Date.now();connection='connected';
  }catch(e){if(requestEpoch===epoch)connection='offline';}
  finally{inflight=false;paint();if(requestEpoch!==epoch)refresh();}
}
function configure(){
...
function configured(){
  epoch++;snapshot=null;received=0;connection='offline';
...
setInterval(refresh,30000);paint();refresh();
})();
