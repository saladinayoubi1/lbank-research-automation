(()=>{
  ...
  const CACHE_KEY='lbank-remote-bundle-v2';
  const ALLOWED=['app.js','personal-tools.js','v3.css','nexus-council.js','nexus-council.css','provider-manager.js'];
  let applied=false;
  ...
  const fetchText=async url=>{const join=url.includes('?')?'&':'?';const r=await fetch(url+join+'t='+Date.now(),{cache:'no-store',redirect:'error'});if(!r.ok)throw new Error('HTTP '+r.status);return r.text()};
  ...
  function execute(text,name){if(document.querySelector(`[data-remote-update="${name}"]`))return;const el=document.createElement('script');el.dataset.remoteUpdate=name;el.textContent=text+'\n//# sourceURL='+name;document.body.appendChild(el)}
