...
def build_vol_adjusted_momentum_targets(
    ...
    realized_vol = log_returns.rolling(volatility_lookback, min_periods=volatility_lookback).std(ddof=0)
...
def _backtest(market_frame: pd.DataFrame, targets: pd.Series, *, initial_cash: float, fee_bps: float, slippage_bps: float) -> dict[str, float | int | bool]:
    result = run_target_exposure_backtest(
        market_frame,
        targets,
        BacktestConfig(initial_cash=initial_cash, fee_bps=fee_bps, slippage_bps=slippage_bps, max_abs_exposure=1.0, liquidate_at_end=True),
    )
...
def _buy_hold_total_return(market_frame: pd.DataFrame, *, initial_cash: float, fee_bps: float, slippage_bps: float) -> float:
    frame = _validated_ohlc(market_frame)
    if len(frame) < 2:
    ...
    execution_frame = pd.DataFrame({"timestamp": [first_timestamp - pd.Timedelta(nanoseconds=1), last_timestamp], "open": [first_open, first_open], "high": [first_open, high], "low": [first_open, low], "close": [first_open, last_close]})
    result = run_target_exposure_backtest(
        execution_frame,
...
def run_vol_adjusted_momentum_robustness(market_frame: pd.DataFrame, *, initial_cash: float = 10_000.0) -> dict[str, object]:
    ...
    for profile_id, profile in EXECUTION_PROFILES.items():
        ...
        benchmark_return = _buy_hold_total_return(market_frame, initial_cash=initial_cash, fee_bps=fee_bps, slippage_bps=slippage_bps)
        profile_runs: list[dict[str, object]] = []
        for momentum_lookback, volatility_lookback, entry_score, exit_score in PARAMETER_GRID:
            metrics = _backtest(
                market_frame,
                build_vol_adjusted_momentum_targets(market_frame, momentum_lookback, volatility_lookback, entry_score, exit_score),
                initial_cash=initial_cash,
                fee_bps=fee_bps,
                slippage_bps=slippage_bps,
            )
            total_return = float(metrics["metric_total_return"])
            row = {"profile_id": profile_id, "momentum_lookback": momentum_lookback, "volatility_lookback": volatility_lookback, "entry_score": entry_score, "exit_score": exit_score, "benchmark_total_return": benchmark_return, "excess_return_vs_buy_hold": total_return - benchmark_return, **metrics}
        ...
        summaries[profile_id] = {"benchmark_total_return": benchmark_return, "median_total_return": median(returns), "worst_total_return": min(returns), "best_total_return": max(returns), "median_excess_return_vs_buy_hold": median(excess), "positive_return_fraction": sum(value > 0.0 for value in returns) / len(returns), "positive_excess_fraction": sum(value > 0.0 for value in excess) / len(excess), "all_variants_inactive": all(value == 0 for value in fills)}
    stress = summaries["stress"]
    kill_conditions = {"all_stress_variants_inactive": bool(stress["all_variants_inactive"]), "all_stress_variants_negative": float(stress["best_total_return"]) < 0.0, "all_stress_variants_trail_buy_hold": float(stress["positive_excess_fraction"]) == 0.0}
    disposition = "reject_hypothesis" if any(kill_conditions.values()) else "continue_to_walkforward_validation"
    return {"schema_version": 1, "hypothesis": "long-only volatility-adjusted time-series momentum on completed 4h bars", "parameter_grid": [{"momentum_lookback": m, "volatility_lookback": v, "entry_score": e, "exit_score": x} for m, v, e, x in PARAMETER_GRID], "execution_profiles": EXECUTION_PROFILES, "runs": runs, "profile_summaries": summaries, "kill_conditions": kill_conditions, "research_disposition": disposition, "authority": "research-backtest-paper-only", "automatic_promotion_allowed": False}
