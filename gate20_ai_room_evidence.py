...
AI_ROOM_EVIDENCE_VERSION = "nexus.gate20-ai-room.v1"
...
def _mission_control(evidence: Mapping[str, Any]) -> dict[str, Any]:
    return {
        ...
        "queue": {"counts": {"READY": 2, "BLOCKED": 0}},
        "agents": ["ai-control", "mission-runner"],
        "runners": ["gate20-e2e"],
        "local_node": "bounded",
        "data": "ready",
        "providers": "local-deterministic",
        "paper": "paper-only" if evidence.get("paper_only") is True else "invalid",
    }


def _write_queue(workspace: Path) -> Path:
    workspace.mkdir(parents=True, exist_ok=True)
    queue = {
        "version": 2,
        "selectionPolicy": {"maxParallelMissions": 2},
        "missions": [
            {
                "id": "G20-FOUNDATION", "title": "Gate 20 foundation", "status": "completed",
                "priority": "automation", "lane": "general", "dependencies": [], "reversible": True,
            },
            {
                "id": "G20-FLOW", "title": "Gate 20 paper flow", "status": "active",
                "priority": "product_research", "lane": "product",
                "dependencies": ["G20-FOUNDATION"], "reversible": True,
            },
            {
                "id": "G20-VERIFY", "title": "Gate 20 verification", "status": "active",
                "priority": "phase_blocker", "lane": "blocker",
                "dependencies": ["G20-FOUNDATION"], "reversible": True,
            },
        ],
    }
    path = workspace / "ai-room-mission-queue.json"
    path.write_bytes(_canonical(queue))
    return path
...
def derive_ai_room_evidence(evidence: Mapping[str, Any], workspace: Path) -> dict[str, Any]:
    ...
    queue_path = _write_queue(workspace)
    memory = _memory()
    ...
    result = {
        ...
        "orchestration": {
            ...
            "tool_status": workflow.get("execution", {}).get("status"),
            "tool_contract_version": workflow.get("execution", {}).get("contract_version"),
            "selected_mission_id": workflow.get("execution", {}).get("selected_mission_id"),
...
def validate_ai_room_evidence(value: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        or orchestration.get("tool_contract_version") != "nexus.mission-runner.v1"
